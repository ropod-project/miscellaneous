...
{
	bool enable;					/*安装角度使能*/
	uint8_t angle_vertical;     	/*垂直方向安装角度（°）*/
}MountingAngleParamTypeDef;

/*解析数据包的类型*/
typedef enum
{
	NULL_PACKET = 0x00,				/*解析数据为空*/
...
typedef struct
{
	SimpleFullRoiDataTypeDef *simple_roi_data;	/*简单ROI数据包*/
...
typedef struct
{
	uint8_t *DeviceName; 				/*当前所有可连接设备的名称(自动筛选)*/
 ...
 * @note
 * @retval	返回设定安装角度变换参数的结果，成功返回 RET_OK（有可能在打包时就出错）
 */
 ...
 * @note
 * @retval	返回获取安装角度变换参数的结果，成功返回 RET_OK（有可能在打包时就出错）
 */
